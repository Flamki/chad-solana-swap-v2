...
export function LazySignInButton({ redirectTo }: { redirectTo: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex min-h-10 items-center rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm font-medium text-white backdrop-blur transition hover:bg-white/[0.08]"
    >
      Sign in
    </button>
  );
